...
uint16_t getSportsPadXY(/*const uint8_t inputPort*/) {
  uint16_t xy;
  ...
  xy = sms1_R << 15 | sms1_L << 14 | sms1_D << 13 | sms1_U << 12; // first 4 bits of X
  sms1_TH.write(HIGH);
  delayMicroseconds(40);
  xy = xy | (sms1_R << 11 | sms1_L << 10 | sms1_D << 9 | sms1_U  << 8); // last 4 bits of X
  sms1_TH.write(LOW);
  delayMicroseconds(40);
  xy = xy | (sms1_R << 7 | sms1_L << 6 | sms1_D << 5 | sms1_U << 4); // first 4 bits of Y
  sms1_TH.write(HIGH);
  delayMicroseconds(40);
  xy = xy | (sms1_R << 3 | sms1_L << 2 | sms1_D << 1 | sms1_U); // last 4 bits of Y
  return xy;
}
...
sportsPadLoop() {
  ...
  bool oldtr = sms1_TR;
  bool oldtl = sms1_TL;
  int8_t oldx, oldy, x, y, dx, dy;

  oldx = x;
  oldy = y;
  uint16_t xy = getSportsPadXY();

  x = (int8_t)(xy >> 8);
  y = (int8_t)(xy & 255);
  dx = -((int8_t)(xy >> 8) - oldx) * 4; // scale the output by 4
  dy = -((int8_t)(xy & 255) - oldy) * 4;
  if (dx || dy || sms1_TL != oldtl || sms1_TR != oldtr) {
    stateChanged = true;
    Mouse.move(dx, dy);
